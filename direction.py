...
def parser():
    p = argparse.ArgumentParser(description = 'Program obliczający długość i zwrot wektora wypadkowego w rzucie na prostą kierunkową wyznaczoną równaniem ax+by+cz=0. UWAGA!!! Wynik nie jest normalizowany lecz podany względem wektora u=[a, b, c]')
    ...
    p.add_argument('-a', '--a', help = 'Współrzędna "a".', type = float, required = True)
    p.add_argument('-b', '--b', help = 'Współrzędna "b".', type = float, required = True)
    p.add_argument('-c', '--c', help = 'Współrzędna "c".', type = float, required = True)       
...
for line in sys.stdin:                  # czytamy całą linię
    ...
    for i in range(0, n):             # iteracja po trójkach liczb ze strumienia wejściowego (teraz już ze zmiennej line)
        ...
        y = l[1 + i*3]
        z = l[2 + i*3]
        t = (p.a*x + p.b*y + p.c*z)/(p.a*p.a+p.b*p.b+p.c*p.c)
        o = o + delim + str(t)
